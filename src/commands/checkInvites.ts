...
export class CheckInvitesCommand implements Command {
  name = 'check-invites';
    ...
    try {
      ...
      if (stats.invites.length === 0) {
        await interaction.reply({
          content: 'You haven\'t created any invites yet. Use `/create-invite` to create your first invite!',
          flags: 64
        });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Active Invites', value: stats.invites.filter(i => !i.expiresAt || i.expiresAt > new Date()).length.toString(), inline: true }
        )
        .setTimestamp();

      if (stats.invites.length > 0) {
        const inviteList = stats.invites.slice(0, 10).map(invite =>
      ...
      await interaction.reply({ content: '❌ Failed to retrieve invite statistics. Please try again.', flags: 64 });
    }
  }
}